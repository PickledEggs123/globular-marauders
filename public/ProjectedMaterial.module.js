...
class ProjectedMaterialModule extends THREE.MeshPhysicalMaterial {
  ...
  set camera(camera) {
    if (!camera || !camera.isCamera) {
      throw new Error('Invalid camera set to the ProjectedMaterialModule');
    }
  ...
  set texture(texture) {
    if (!texture?.isTexture) {
      throw new Error('Invalid texture set to the ProjectedMaterialModule');
    }
  ...
  set backgroundOpacity(backgroundOpacity) {
    ...
    if (backgroundOpacity < 1 && !this.transparent) {
      console.warn('You have to pass "transparent: true" to the ProjectedMaterialModule for the backgroundOpacity option to work');
    }
  }
  get cover() {
  ...
  constructor(_temp) {
    ...
    if (!texture.isTexture) {
      throw new Error('Invalid texture passed to the ProjectedMaterialModule');
    }
    if (!camera.isCamera) {
      throw new Error('Invalid camera passed to the ProjectedMaterialModule');
    }
...
export { allocateProjectionData, ProjectedMaterialModule as default };
